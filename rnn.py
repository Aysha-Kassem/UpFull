...
# ==============================
# 3) BUILD BiGRU + ATTENTION MODEL
# ==============================
def build_bigru_attention(time_steps, features):
    inp = Input(shape=(time_steps, features))
    x = layers.Bidirectional(layers.GRU(128, return_sequences=True))(inp)  # <- Apply on inp
...
# ==============================
def train_bigru_attention():
    X_train, X_test, y_train_now, y_test_now, y_train_soon, y_test_soon = prepare_common_data()
    
    model = build_bigru_attention(X_train.shape[1], X_train.shape[2])
    print(model.summary())

    callbacks = [
        EarlyStopping(monitor='val_loss', patience=8, restore_best_weights=True),
        ReduceLROnPlateau(monitor='val_loss', factor=0.4, patience=4),
        ModelCheckpoint("BEST_BiGRU_Attention_lstm(2).keras", save_best_only=True, monitor='val_loss')
    ]
    ...
    model.save("FINAL_BiGRU_Attention_lstm(2).keras")
